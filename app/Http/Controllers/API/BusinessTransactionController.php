...
class BusinessTransactionController extends Controller
{
    ...
    public function updateProduct(Request $request, $id)
    {
        ...
        // Optionally, validate incoming request data here
        $validated = $request->validate([
            'name' => 'sometimes|string',
            'description' => 'sometimes|string',
            'business_code' => 'sometimes|string',
...
public function deleteBusinessTransaction($id)
{
    $transaction = BusinessTransaction::find($id);

    if (!$transaction) {
        return response()->json(['message' => 'Business Transaction not found'], 404);
    }
    ...
    return response()->json(['message' => 'Business Transaction deleted successfully']);
}
...
public function getAllInvoiceByBusinessght($businessCode)
{
    try {
        $getUser = Auth::user();
        if (!$getUser) {
...
public function getAllInvoiceByBusiness($businessCode)
{
    try {
        ...
        // Retrieve business transactions for the authenticated user and specific business code, filtering by presence of an invoice number, ordered by invoice number (recent first)
        $getAllInvoiceByABusiness = businessTransaction::where('owner_id', $getUser->id)
            ->where('business_code', $businessCode)
            ->whereNotNull('invoice_number')
            ->orderBy('created_at', 'desc') // Order by most recent first based on creation date
            ->get();

        // Filter out transactions where the name property is 'MPOS'
        $filteredData = $getAllInvoiceByABusiness->filter(function ($transaction) {
            return $transaction->name !== 'MPOS';
        });

        // Group the transactions by product code
        $groupedData = $filteredData->groupBy('product_code');
...
public function getAllIinkByBusiness($businessCode)
{
    try {
        ...
        // Retrieve business transactions for the authenticated user and specific business code, ordered by creation date (recent first)
        ...
        // Filter out transactions where the name property is 'MPOS'
        $filteredData = $getAllIinkByBusiness->filter(function ($transaction) {
            return $transaction->name !== 'MPOS';
        });
...
public function getAllIinkByBusinessgh($businessCode)
    {
        try {
            $getUser = Auth::user();
            if (!$getUser) {
            ...
            if (count($uniqueCodes) === 1 && count($quantities) === 1) {
			  ...
			  Mail::to($user->customer_email, $busName, $cusName)->send(new BusinessPaylinkMail($paylink, $busName, $cusName));
			    ...
			    'due_date' => $current->addDays($request->due_days),
                    ...
                    //   $filename = 'image/invoice_' . time() . '.pdf'; // saved in storage/app/public/image/
                    //     \Storage::disk('public')->put($filename, $pdf->output());
                    
                    $filename = 'image/invoice_' . time() . '.pdf'; // saved in storage/app/public/image/
                        \Storage::disk('public')->put($filename, $pdf->output());
                        
                        // Manually build the correct URL to match subfolder structure
                        $pdf_url = url('Azat_pay/storage/app/public/' . $filename);
                        
                      //  $pdf_url = asset('storage/' . $filename); // publicly accessible URL
 ...
 *
 * This endpoint fetches all MPOS transactions associated with the authenticated user and a given business code.
 * The transactions are grouped by `product_code` and `email`, and the total amount paid for each group is calculated.
 *
 * @authenticated
 *
 * @responseField data array An array of grouped transaction data.
 * @responseField total integer The total number of groups returned.
 *
 * @param string $businessCode The business code for which transactions are to be retrieved.
 *
 * @response {
 *   "data": [
 *     {
 ...
 *       "data": [
 *         {
 *           // Transaction fields such as id, amount, etc.
 *         }
 *       ]
 *     }
 *   ],
...
public function getMposPerBusiness($businessCode): array
{
    try {
        // Get the authenticated user
        $getUser = Auth::user();
        ...
        // Retrieve all transactions for the authenticated user and specific business code
        $transactions = DB::table('business_transactions')
            ->where('owner_id', $getUser->id)
            ->where('business_code', $businessCode)
            ->where('name', 'MPOS')
            ->orderBy('created_at', 'desc') // Ensure the latest created_at is returned first
            ->get();

        // Debug: Log the raw transactions
      //  \Log::info('Raw transactions: ', (array) $transactions->toArray());

        if ($transactions->isEmpty()) {
            // Return early if no transactions are found
            return [
                'data' => [],
                'total' => 0,
            ];
        ...
        // Separate transactions with null product_code
        $transactionsWithNullProductCode = $transactions->whereNull('product_code');
        $transactionsWithProductCode = $transactions->whereNotNull('product_code');

        // Group transactions with product_code by product_code and email
        $groupedTransactionsWithProductCode = $transactionsWithProductCode->groupBy(function ($item) {
            return $item->product_code . '|' . $item->email; // Concatenate product_code and email
        });

        // Prepare the response format for transactions with product_code
        $formattedDataWithProductCode = $groupedTransactionsWithProductCode->map(function ($items, $key) {
            $keyParts = explode('|', $key);
            $product_code = $keyParts[0];
            $email = $keyParts[1];

            // Debug: Log the details for each group
            // \Log::info('Processing group with product code:', [
            //     'Product Code' => $product_code,
            //     'Email' => $email,
            //     'Transaction Count' => $items->count(),
            //     'Total Amount' => $items->sum('transaction_amount')
            // ]);

            return [
                'Product Code' => $product_code,
                'email' => $email,
                'total_Amount' => $items->sum('transaction_amount'),
                'data' => $items->map(function($item) {
                    return (array) $item; // Cast each transaction to an array
                })->values()->toArray(), // Return all fields for each transaction
            ];
        });

        // Prepare the response format for transactions with null product_code
        $formattedDataWithNullProductCode = $transactionsWithNullProductCode->map(function ($item) {
            // Debug: Log the details for each transaction with null product_code
            // \Log::info('Processing transaction with null product code:', [
            //     'Email' => $item->email,
            ...
            return [
                ...
                'data' => [(array) $item], // Wrap the item in an array and cast to an array
            ];
        });

        // Merge both formatted data sets
        $flattenedData = $formattedDataWithProductCode->values()->merge($formattedDataWithNullProductCode)->all();

        // Sort the merged data to ensure the latest created_at is returned first
        usort($flattenedData, function($a, $b) {
            return strtotime($b['data'][0]['created_at']) - strtotime($a['data'][0]['created_at']);
        });

        // Debug: Log the final formatted data
       // \Log::info('Final formatted data: ', $flattenedData);
