...
//	    'uidd'=> Str::random(10),
         ...
         $payers[] =  ["payerEmail" => $em, "paymentAmount" => $info->payable, "payerName" => $user ? $user->first_name . ' ' . $user->last_name : 'Unknown'];
...
public function getUserExpense(Request $request)
{
    ...
    $getUserExpense = DB::table('expenses')
       ...
       // ->whereNotNull('user_expenses.residualAmount')
...
//     // Query expenses with aggregation and explicit fields
//     $getUserExpense = DB::table('expenses')
//         ->join('user_expenses', function ($join) {
//             $join->on('expenses.user_id', '=', 'user_expenses.principal_id')
//                  ->on('expenses.id', '=', 'user_expenses.expense_id');
...
//             'expenses.updated_at',
//             'expenses.confirm',
//             DB::raw('SUM(user_expenses.residualAmount) as total_paid')
//         )
//         ->whereIn('expenses.id', $expenseIds)
//         ->whereNull('expenses.deleted_at')
//         ->where('expenses.confirm', '=', 1)
...
//     // Query transactions user was invited to
//     $getUserExpenseAddedTransactions = UserExpense::where('email', $getAuthUser->email)
//         ->whereNull('deleted_at')
//         ->orderBy('created_at', 'desc')
//         ->paginate($perPage, ['*'], 'page', $page);
