    ...
    public function updateBalanceResidual()
{
    try {
        ...
        $updatedAt = Carbon::parse($userExpense->updated_at); // Explicitly parse to Carbon
        $updateTable = 'userExpense';
    } elseif ($userGroup) {
        $residualAmount = $userGroup->residualAmount;
        $updatedAt = Carbon::parse($userGroup->updated_at); // Explicitly parse to Carbon
        $updateTable = 'userGroup';
    } elseif ($businessTransaction) {
        $residualAmount = $businessTransaction->residualAmount;
        $updatedAt = Carbon::parse($businessTransaction->updated_at); // Explicitly parse to Carbon
		...
		if ($updatedAt && $updatedAt->diffInSeconds(Carbon::parse($latestPayment->updated_at)) <= 3600){
                ...
                Log::error("Updated_at values are not within 1 hour.");
            }
        } else {
	...
	$this->voucherService->initiatePayout();
